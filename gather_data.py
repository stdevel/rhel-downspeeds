...
def gather_rhel(release, options):
    ...
    :type release: int
    """
    # https://access.redhat.com/hydra/rest/search/kcs?q=*:*&start=0&fq=portal_advisory_type:(%22Security%20Advisory%22)%20AND%20documentKind:(%22Errata%22)&fq=portal_product_filter:Red\%20Hat\%20Enterprise\%20Linux|*|8|*&facet.mincount=1&rows=5000&fl=id,portal_severity,portal_product_names,portal_publication_date,portal_synopsis,view_uri,allTitle&sort=portal_publication_date%20desc&p=1
...
def gather_almalinux(release, options):
    ...
    if options.use_cache and check_almalinux_file(_file, options):
        pass
    else:
        # download file
...
def check_almalinux_file(file, options):
    """
...
def gather_rockylinux(release, options):
    ...
    if options.use_cache and check_rockylinux_file(_file, options):
        pass
    else:
        # download file
        _errata = []
        ...
        try:
            ...
            LOGGER.debug("Found %s errata in %s pages", _hits['total'], _pages)
            i = 0
            while i < _pages:
                LOGGER.debug("Loading errata #%s", i+1)
                _hits = json.loads(requests.get(f"{_url}&page={i}&limit={_limit}").text)
                print(_hits['advisories'])
                _errata.append(_hits['advisories'])
                i=i+1
            #
            print(len(_errata))
            # print(_errata)
...
def check_rockylinux_file(file, options):
    """
